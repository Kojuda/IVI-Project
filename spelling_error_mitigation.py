...
dict_alphabet['o']='o{0,2}O{0,2}'
dict_alphabet['p']='p{0,2}P{0,2}'
dict_alphabet['q']='q{0,2}Q{0,2}'
dict_alphabet['r']='r{0,2}R{0,2}'
dict_alphabet['s']='s{0,2}S{0,2}'
dict_alphabet['t']='t{0,2}T{0,2}'
...
def word_to_regex(word: str):
    ...
    for i in range(0, len(word)):
       l.append("("+dict_alphabet[word[i]]+"){1}")
    res= seperator.join(l)
    return res


if __name__ == '__main__':
    test_1 = 'hdegegfgfe pakrrot gezdlzted'
    test = 'parrot'
    res = word_to_regex(test)
    print(res, type(res))
    print(re.search(res, 'parrrot'))